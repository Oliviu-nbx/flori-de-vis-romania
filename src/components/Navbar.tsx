      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-floral-100 absolute w-full left-0 shadow-lg animate-fade-in">
          <div className="px-4 pt-2 pb-6 space-y-4">
            <Link to="/" className="block py-2.5 px-4 rounded font-medium text-floral-800 hover:bg-floral-50" onClick={() => setIsMenuOpen(false)}>
              Acasă
            </Link>
            <Link to="/colectii" className="block py-2.5 px-4 rounded font-medium text-floral-800 hover:bg-floral-50" onClick={() => setIsMenuOpen(false)}>
              Colecții
            </Link>
            <Link to="/ocazii" className="block py-2.5 px-4 rounded font-medium text-floral-800 hover:bg-floral-50" onClick={() => setIsMenuOpen(false)}>
              Ocazii
            </Link>
            <Link to="/despre" className="block py-2.5 px-4 rounded font-medium text-floral-800 hover:bg-floral-50" onClick={() => setIsMenuOpen(false)}>
              Despre Noi
            </Link>
            <Link to="/contact" className="block py-2.5 px-4 rounded font-medium text-floral-800 hover:bg-floral-50" onClick={() => setIsMenuOpen(false)}>
              Contact
            </Link>
          </div>
        </div>
